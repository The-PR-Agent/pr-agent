...
| `claude_adaptive_thinking_models_override` | [] | List model ids to additionally treat as adaptive-only Claude models. Use the exact request model id for opaque Bedrock application inference profile ARNs so PR-Agent and LiteLLM both keep the adaptive payload. Built-in named-model detection remains enabled. |
