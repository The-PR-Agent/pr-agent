...
| `description_issue_regex` | "" | Optional: regex replacing bare #N references in PR descriptions, with exactly one capturing group for the issue number. Empty or invalid patterns use the default bare #N matching (up to six digits); invalid patterns log a warning. Custom patterns control their own digit limit. Full issue URLs and owner/repo#N references remain supported. Example (TOML literal quotes preserve backslashes): description_issue_regex = '(?i)(?:fixes\|closes\|resolves)\s+#(\d+)' |
