...
class GitProvider(ABC):
    ...
    def _is_generated_by_pr_agent(self, description_lowercase: str) -> bool:
        # Fast path: if the description carries the hidden HTML comment we
        # inject at write time, it was generated by pr-agent.  The comment is
        # invisible in rendered markdown but reliable for machine detection.
        if '<!-- pr-agent-generated -->' in description_lowercase:
            return True
        # Standard path: match the header list at the start of the body.
        # Only startswith is used here because matching visible headers
        # anywhere in the body misclassifies human descriptions that quote
        # our output (e.g. "> ### **PR Type**" in a blockquote).
        possible_headers = self._possible_headers()
