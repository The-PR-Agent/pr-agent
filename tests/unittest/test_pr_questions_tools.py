...
from pr_agent.tools.pr_questions import READ_PR_FILE_TOOL, PRQuestions
...
class TestExecuteReadPrFileContent:
    ...
    def test_azure_devops_leading_slash_path_matching(self):
        file_azure = FilePatchInfo(
            base_file="old",
            head_file="azure content\n",
            patch="",
            filename="/src/azure_file.py",
            edit_type=EDIT_TYPE.MODIFIED,
            head_file_is_complete=True,
        )
        pr = _make_pr_questions(diff_files=[file_azure])
        res = json.loads(pr._execute_read_pr_file('{"path": "src/azure_file.py"}'))
        assert "error" not in res
        assert res["path"] == "/src/azure_file.py"
        assert res["content"] == "azure content\n"
...
class TestOrchestration:
    ...
    async def test_successful_tool_round_to_turn2(self):
        ...
        assert turn2_call.kwargs["tools"] == [READ_PR_FILE_TOOL]
    ...
    async def test_turn2_requesting_additional_tools_without_content_falls_back_to_text(self):
        file_target = FilePatchInfo(
            base_file="",
            head_file="content",
            patch="",
        ...
        pr.ai_handler.chat_completion_with_tools = AsyncMock(side_effect=[turn1, turn2])
        pr.ai_handler.chat_completion = AsyncMock(return_value=("Fallback text answer", "stop"))

        ans = await pr._get_prediction_with_tools("gpt-4o", "sys", "usr")
        assert ans == "Fallback text answer"
        assert pr.ai_handler.chat_completion_with_tools.call_count == 2
        pr.ai_handler.chat_completion.assert_called_once()
...
class TestPromptTemplateToolsConditional:
    def test_prompt_template_renders_tool_instructions_only_when_enabled(self):
        ...
        rendered_disabled = env.from_string(template_str).render(vars_disabled)
        assert "If you are provided with a tool to read file contents" not in rendered_disabled
