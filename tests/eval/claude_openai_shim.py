...
adapts it: system+user are concatenated and piped to `claude -p` on stdin, and the
...
Usage: CLAUDE_SHIM_UNUSED=... python3 claude_openai_shim.py --port 8899 --model gemini-3.7-flash-high
"""
import argparse
import json
import os
...
def _call_claude(prompt: str) -> tuple[str, dict]:
    cmd = [
        ...
        "--output-format", "json",
        "--model", ARGS.model,
        # The reviewer under test must answer from the prompt alone. Claude Code ships with file
        # and shell tools; left enabled it could read the repository it is reviewing and score for
        # context the harness never sent, which would make the row meaningless.
        "--disallowed-tools", "Bash", "Read", "Glob", "Grep", "Edit", "Write", "WebFetch", "WebSearch",
    ]
    started = time.time()
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"claude exited {proc.returncode}: {proc.stderr.decode()[:600]}")
    ...
    for line in raw.splitlines():
        ...
        if line.startswith("{"):
            try:
                ...
                continue
    if envelope is None:
        raise RuntimeError(f"no JSON envelope in claude output: {raw[:600]}")
    if envelope.get("is_error"):
        raise RuntimeError(f"claude reported an error: {raw[:600]}")
...
def main() -> int:
    ...
    parser.add_argument("--model", default="gemini-3.7-flash-high",
                        help="claude CLI model alias or id (e.g. opus, sonnet)")
    parser.add_argument("--claude-bin", default="claude")
    parser.add_argument("--timeout", type=int, default=1800)
    parser.add_argument("--log", default="")
    parser.add_argument("--raw-dir", default="", help="dump each prompt and raw result here")
    ARGS = parser.parse_args()
    # No API key check: the claude CLI carries its own authentication.
    EMPTY_WORKSPACE = tempfile.mkdtemp(prefix="claude-shim-empty-")
    server = ThreadingHTTPServer(("127.0.0.1", ARGS.port), Handler)
    print(f"claude shim on http://127.0.0.1:{ARGS.port}/v1 model={ARGS.model} workspace={EMPTY_WORKSPACE}",
          flush=True)
